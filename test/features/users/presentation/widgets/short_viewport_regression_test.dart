...
void main() {
  ...
  Widget error() =>
      const ErrorView(failure: NetworkFailure(), onRetry: _noop);
  ...
  for (final MapEntry<String, Widget Function()> entry in states.entries) {
    for (final Size size in viewports) {
      testWidgets(
        '${entry.key} does not overflow at ${size.width.toInt()}x'
        '${size.height.toInt()}',
        (WidgetTester tester) async {
          ...
          await tester.pumpWidget(
            wrapForTest(Scaffold(body: entry.value())),
          );
          await tester.pump();
