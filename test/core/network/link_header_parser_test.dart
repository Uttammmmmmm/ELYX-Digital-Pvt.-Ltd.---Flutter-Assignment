...
void main() {
  group('LinkHeaderParser.nextSince', () {
    test('extracts the since cursor from a real GitHub header', () {
      ...
      expect(LinkHeaderParser.nextSince(header), 46);
    });

    test('returns null when the header is absent or blank', () {
      expect(LinkHeaderParser.nextSince(null), isNull);
      expect(LinkHeaderParser.nextSince('   '), isNull);
    });
    ...
    test('is not confused by commas inside the URL', () {
      ...
      expect(LinkHeaderParser.nextSince(header), 7);
    });

    test('degrades to null on a malformed header instead of throwing', () {
      expect(() => LinkHeaderParser.nextSince('garbage;;;'), returnsNormally);
      expect(LinkHeaderParser.nextSince('garbage;;;'), isNull);
    });
