...
///
/// `github` is the default because it is the only one of the two that serves
/// LIVE data. `reqres` returns the same 12 invented users on every request, so
/// on a device it is indistinguishable from hardcoded placeholder content --
/// it is a test fixture, and must be opted into explicitly.
...
/// a build flag should not be a launch crash. The fallback is deliberately the
/// LIVE source -- a typo that silently serves a static fixture is far harder to
/// notice than one that fails loudly. Keep this in lockstep with
/// [ApiSource.fromName], which resolves the base URL and headers: if the two
/// disagree, DioClient points at one host while the API builds paths for the
/// other.
